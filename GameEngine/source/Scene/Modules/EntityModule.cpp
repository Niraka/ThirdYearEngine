...
m_sXMLDEModelName("model_name")
{
	...
	m_xmlDescriptor.addElement(m_sXMLDEAI);
	...
	for (; eCrnt != nullptr; eCrnt = eCrnt->NextSiblingElement())
	{
		...
		if (eCrnt->FirstChildElement() == nullptr)
		{
			...
			if (m_xmlDescriptor.hasElement(sName))
			{
				...
				else if (sName == m_sXMLDEAI)
				{
					spec.bUseAi = m_stringUtils.toBool(sText);
				}
...
unsigned int EntityModule::createEntity(const EntitySpecification& spec, const std::string& sGroupName)
{
	...
	if (spec.bUseAi && m_aiModule)
	{
		//entity->attachAINode(m_aiModule->createAINode());
	}
...
void EntityModule::moduleDependencyBecameAvailable(Modules moduleName, std::shared_ptr<void> module)
{
	...
	else if (moduleName == Modules::WINDOW)
	{
		...
		if (windowModule)
		{
			...
			std::shared_ptr<Scene> scene = m_windowModule->getScene();
			if (scene)
			{
				scene->addRenderable(this, 17, RenderModes::PER_CAMERA);
			}
		}
	}
	else if (moduleName == Modules::ACTING)
	{
		std::shared_ptr<ActingModule> actingModule = std::static_pointer_cast<ActingModule>(module);
		if (actingModule)
		{
			m_actingModule = actingModule;
		}
	}
	...
	else if (moduleName == Modules::AI)
	{
		m_aiModule = std::static_pointer_cast<AIModule>(module);
	}
}
