...
FunctionSetPointLightPosition::FunctionSetPointLightPosition(std::vector<ScriptParam>& params) :
...
FunctionSetPointLightPosition::~FunctionSetPointLightPosition()
{
}

ScriptVar& FunctionSetPointLightPosition::execute(ScriptInfoPack& pack)
{
	if (pack.windowModule)
	{
		...
		if (scene)
		{
			std::shared_ptr<PointLight> light = scene->getPointLight(m_lightId.getValue(pack).getInt());
			if (light)
			{
